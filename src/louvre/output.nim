...
func refreshRateLimit*(
  output: Output
): int32 {.importcpp: "Louvre::LOutput::refreshRateLimit".}
proc `scale=`*(
  output: ptr Output, scale: float32
) {.importcpp: "Louvre::LOutput::setScale".}

proc `leasable=`*(
  output: ptr Output, leasable: bool
) {.importcpp: "Louvre::LOutput::setLeasable".}

proc `position=`*(
  output: ptr Output, pos: Point
) {.importcpp: "Louvre::LOutput::setPos".}

proc setVsyncInternal(
  output: ptr Output, enabled: bool
): bool {.importcpp: "Louvre::LOutput::enableVSync".}

proc `refreshRateLimit=`*(
  output: ptr Output, hz: int32
) {.importcpp: "Louvre::LOutput::setRefreshRateLimit".}
...
func manufacturerInternal(
  output: Output
): cstring {.importcpp: "Louvre::LOutput::manufacturer".}
func descriptionInternal(
  output: Output
): cstring {.importcpp: "Louvre::LOutput::description".}

proc constructOutput*[T](
  typ: typedesc[T], params {.immutable.}: pointer
): ptr T {.importcpp: "('1*)(new Louvre::LOutput(@))".}
...
proc `vsync=`*(
    output: ptr Output, enabled: bool
) {.inline, raises: [CannotSetVSyncState].} =
